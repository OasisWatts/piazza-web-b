...
let setting: typeof import("util/setting")
...
export class Logger {
      ...
      public static async initialize(subject: string): Promise<void> {
            fs = await import("fs")
            setting = await import("util/setting")
            ...
            } else Logger.errorSignificant().put("Log files won't be generated.").out()
      }
       ...
       * @param title 제목.
       */
      public static errorSignificant(title?: string): Logger {
       ...
       * @param title 제목.
       */
      public static errorApp(c: ErrorCode): Logger {
            return new Logger(LogLevel.APP_ERROR, ErrorCode[c])
      }
      /**
       * 앱 패스 로그 인스턴스.
       *
       ...
       * @param title 제목.
       */
      public static passSignificant(title?: string): Logger {
      ...
      private static getLocalISODate(): string {
            const now = new Date()
            const offset = -Math.round(TIMEZONE_OFFSET / UnitDate.HOUR) || ""
      ...
      constructor(type: LogLevel = LogLevel.APP_PASS, title: string = "") {
      ...
      private getText(): string {
            ...
            return [
                  this.list[0][1],
                  ...this.list.slice(1).map(([head, body], i) => `${prefix}${Logger.escape(LogStyle.LINE)}${i === last ? "└" : "├"}─ ${(head ?? String(i)).padEnd(maxDigit, " ")}${Logger.escape()}: ${body}`),
            ].join("\n")
      }

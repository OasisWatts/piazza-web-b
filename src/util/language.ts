import { SETTINGS, fileInPublic } from "./setting"
...
export function getLocale(req: Express.Request): string {
      let R: string = req.cookies["brownie.locale"]
...
export function loadLanguages(): void {
      const prototables = reduceToTable(LANGUAGE_SUPPORT, (v) => (
            JSON.parse(fileInPublic(`lang/${v}.json`).toString()) as Table<Table<string> & { "$include"?: string[] }>
      ))
